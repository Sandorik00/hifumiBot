module.exports = {
    ...
    async run(message, args) {
        ...
        while (true) {
            ...
            if (args[2]) {
                sum_messages = sum_messages.filter(
                    (m) => m.author.username == args[2]
                );
            }

            if (
                sum_messages.length >= 1000 ||
                messages.size != 100 ||
                sum_messages.length >= args[1]
            ) {
                break;
            }
        }
        ...
        let confirmMessage = await message.reply(
            sum_messages[sum_messages.length - 1]['url']
        );

        await confirmMessage.react('✅');
        await confirmMessage.react('❌');

        let collector = confirmMessage.createReactionCollector(filter, {
            time: 120000,
            dispose: true,
        });
