...
const forParadox = ["Если ещё раз выйдешь то бан!!", "Ты совсем дурачок или как?", "Слушай, ну хватит уже...", "Даже если ты не Парадокс, то всё равно бан.", "Sandorik, да вломи ты ему наконец!!"];
...
client.on('guildMemberAdd', async (member: GuildMember) =>
{
    ...
    if(member.id === "641366682761166860") {
        ...
        (HelloChannel as TextChannel).send(forParadox[Math.floor(Math.random() * forParadox.length)]);
    } else {
    ...
    {
        (HelloChannel as TextChannel).send("Привет, " + `${member}!! ` + `Судя по моим записям, ты воспользовался инвайтом - "**${from}**".`);
    }
